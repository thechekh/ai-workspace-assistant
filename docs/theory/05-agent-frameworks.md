...
agent/backends/
  ...
  langgraph.py   282 lines  LangGraph runtime
```
...
- The **same WebSocket test suite runs ×3** — identical assertions pass on
  every runtime (72 tests total include the whole suite parametrized).
...
2. **Model abstraction built in** — `groq:...`, `openai:...` strings or
...
4. It's coupled to LangChain's model/message types, so we wrote a ~95-line
   ...
   payoff: FakeLLM, Groq, and even the scripted test LLMs run on LangGraph
