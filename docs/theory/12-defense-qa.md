...
|---|---|---|
| Tests | 629 backend tests + 92 frontend tests | `uv run pytest -q`, `npm run test:run` |
...
110/364/318 lines; everything else (tools, memory, telemetry, protocol) is
...
**629 tests in ~23 seconds, fully offline** — no network, no containers, no
