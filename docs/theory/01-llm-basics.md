...
OpenAI, Groq (free tier, serves open models like Llama fast), Ollama (models
...
- All 204 tests run in seconds, offline, at $0 — no flaky network, no burnt
...
value. Dev/demo runs offline on a deterministic fake; Groq's free tier
(Llama 3.3 70B) for a real model; an OpenAI mini-tier model for final
quality testing. The architecture doesn't care.
