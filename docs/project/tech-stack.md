...
| LLM (dev) | **Groq free tier** (+ Ollama local fallback) | Gemini free tier, OpenRouter `:free`, xAI Grok credits | Free, fast, OpenAI-compatible API, models with solid tool calling |
...
class Settings(BaseSettings):
    ...
    llm_provider: Literal["groq", "ollama", "gemini", "openai"] = "groq"
...
| B | **Pydantic AI** | The modern typed framework: tools as decorated functions with validated args, built-in provider abstraction (Groq/OpenAI/Gemini/Ollama by model string), native MCP client support, native Logfire instrumentation, streaming. |
...
- **Groq** (groq.com) — an *inference provider* running open models (Llama 3.3 70B, Llama 3.1 8B, Qwen…) on custom hardware. Generous **free tier**, very fast, OpenAI-compatible endpoint. This is the recommended free option.
...
| Provider | Cost | Notes |
|---|---|---|
| **Groq** ✅ | Free tier (rate-limited) | `llama-3.3-70b-versatile` has solid tool calling — good enough to develop the whole agent loop |
...
| Google Gemini (AI Studio) | Generous free tier | `gemini-*-flash` models; good tool calling; second option if Groq limits bite |
...
| 1 | WS chat endpoint + Redis sessions + streaming with **Groq free tier**; minimal Vue chat page |
...
Python 3.12 · uv · ruff · pyright · FastAPI · uvicorn · Pydantic v2 · pydantic-settings · WebSockets · custom-loop / Pydantic AI / LangGraph (comparative) · Groq free tier → OpenAI mini ($25) · text-embedding-3-small → voyage-3 (comparative) · Qdrant (+hybrid+rerank) · Redis (+summarization) · taskiq · MCP (FastMCP + official servers) · Logfire + Langfuse over OpenTelemetry · pytest/respx + golden evals · Vue 3 + Vite + TS · Docker Compose.
