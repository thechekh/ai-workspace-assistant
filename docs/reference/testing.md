...
## Tier C — real model (OpenAI key)

```sh
# .env: ASSISTANT_LLM_PROVIDER=openai, ASSISTANT_LLM_API_KEY=sk-...
...
- [ ] **Real token usage**: stats line shows `N→M tok` **without** `(est)` —
      OpenAI reports usage via `stream_options.include_usage`.
- [ ] **Cost figure**: `~$0.000X` appears (indicative at listed per-token
      prices; OpenAI free tier actually bills $0). Also
...
- [ ] **llama tool-syntax salvage**: llama-3.3 sometimes emits its tool call
      as text (`<function...>`) or trips OpenAI's `tool_use_failed` error.
...
- [ ] **Rate-limit UX**: hammer 5–6 messages quickly. OpenAI free tier limits
      `gpt-4.1-nano` per minute (≈30 req, ≈6k tokens) **and per
      ...
      retry…` in server logs); on exhaustion the chat shows OpenAI's own
      message (which limit + how long to wait), never a generic server
      error. `/metrics`: `assistant_errors_total{kind="rate_limited"}`.
      Daily budget gone? Set `ASSISTANT_LLM_MODEL=gpt-4o-mini`
