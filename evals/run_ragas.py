...
claims, then verify each). All 18 questions is roughly 200 calls — more than a
Groq free-tier day on llama-3.3-70b. Start with `--limit 3`, or point
ASSISTANT_LLM_MODEL at llama-3.1-8b-instant.
"""
...
def judge(dataset: list[dict[str, object]], settings: Settings) -> dict[str, float]:
    ...
    result = evaluate(
        ...
        # which matters because Groq serves no embeddings API.
