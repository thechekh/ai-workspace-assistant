...
def test_env_prefix_override(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("ASSISTANT_LLM_PROVIDER", "groq")
    monkeypatch.setenv("ASSISTANT_LLM_API_KEY", "gsk_test")
    settings = HermeticSettings()
...
def test_build_llm_requires_key_for_hosted_providers():
    settings = HermeticSettings(llm_provider="groq")
    with pytest.raises(ValueError, match="ASSISTANT_LLM_API_KEY"):
        build_llm(settings)


def test_build_llm_groq_uses_openai_compatible_client():
    settings = HermeticSettings(llm_provider="groq", llm_api_key=SecretStr("gsk_test"))
