...
# --- provider "tool_use_failed" mid-stream flake -------------------------------
...
async def test_stream_step_retries_tool_use_failure():
    llm = OpenAICompatibleLLM(model="m", api_key="k", base_url="https://api.test/v1")
...
def test_estimate_cost_usd_uses_the_price_table():
    # gpt-4.1-nano (the default): $0.10 / $0.40 per 1M tokens
    assert estimate_cost_usd("gpt-4.1-nano", 1_000_000, 0) == pytest.approx(0.10)
    assert estimate_cost_usd("gpt-4.1-nano", 0, 1_000_000) == pytest.approx(0.40)
    assert estimate_cost_usd("gpt-4.1-nano", 1000, 200) == pytest.approx(0.00018)
